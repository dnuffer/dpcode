...
# TODO: write format_table which puts each row on a new line and spaces out the columns so that each column is "auto-sized"
...
def time_set_difference_implementations():
  ...
  def time(description, f):
    table.append([description,
                  str(
                    timeit.timeit(
                      lambda: 
                        calculate_set_difference(f, range1, range2), 
                      number=repeat) \
                    / repeat * ns_per_sec) + "ns"
                  ])
