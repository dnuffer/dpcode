...
logistic.regression.cost = function(theta, X, y) {
  m = length(y)

  J = (-1 / m) * (t(log(sigmoid(X %*% theta))) %*% y + t(log(1 - sigmoid(X %*% theta))) %*% (1 - y))
...
test_that("logistic.regression.cost works", {
          theta = matrix(c(0, 0, 0), nrow=3, ncol=1, byrow=TRUE)
          ...
          y = matrix(c(0, 0, 0, 1), nrow=4, ncol=1, byrow=TRUE)
          res = logistic.regression.cost(theta, X, y)
