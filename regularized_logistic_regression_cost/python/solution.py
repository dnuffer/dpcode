...
def cost_function_reg(theta, X, y, lambda_reg):
  ...
  sigmoid_X_theta = sigmoid(dot(X, theta))

  J = (-1. / m) * ( dot(log(    sigmoid_X_theta).T,       y) + \
                    dot(log(1 - sigmoid_X_theta).T, (1 - y)) \
                  ) \
      + \
      lambda_reg / (2. * m) * dot(theta_reg.T, theta_reg)

  grad = (1. / m) * dot(X.T, (sigmoid_X_theta - y)) + \
         (lambda_reg / m) * theta_reg
